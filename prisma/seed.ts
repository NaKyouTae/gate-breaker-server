...
import { purgeNonWeaponEquipment, migrateOldWeapons, seedWeaponCatalog } from '../src/bootstrap/weapon-catalog.seed';
...
async function main() {
  ...
  // Weapons: 기존 무기 마이그레이션 + 카탈로그 시드
  const migration = await migrateOldWeapons(prisma);
  console.log(`Old weapons migrated: ${migration.migrated}`);

  const weaponSeed = await seedWeaponCatalog(prisma);
  console.log(`Weapon catalog: created=${weaponSeed.created}, updated=${weaponSeed.updated}`);
